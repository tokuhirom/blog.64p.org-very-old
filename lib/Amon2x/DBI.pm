...
sub prepare {
    ...
    my $sth = $self->SUPER::prepare(@args) or do {
        Amon2x::DBI::Util::handle_error($_[1], [], $self->errstr);
    };
    $sth->{private_sql} = $_[1];
    return $sth;
}

package Amon2x::DBI::dr;
use base qw/DBI::dr/;

package Amon2x::DBI::st;
use base qw/DBI::st/;
...
sub execute {
    ...
    $self->SUPER::execute(@args) or do {
        Amon2x::DBI::Util::handle_error($self->{private_sql}, \@args, $self->errstr);
    };
}

sub sql { $_[0]->{private_sql} }

package Amon2x::DBI::Util;
use Carp::Clan qw{^(DBI::|Amon2x::DBI::|DBD::)};
use Data::Dumper ();
